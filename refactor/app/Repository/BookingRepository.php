...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => [], 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => 0, 'pagenum' => $pagenum];
    }
    /**
     * @param $user
     * @param $data
    ...
    public function store($user, $data)
    {
        ...
        if ($consumer_type == 'rwsconsumer')
            ...
            $data['job_type'] = 'paid';
        $data['b_created_at'] = now()->toDateTimeString();
        if (isset($due))
            ...
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                ...
                foreach ($jobs as $oneJob) {
                    if ($job->id == $oneJob->id) { // one potential job is the same with current job
                        ...
                        if ($job_for_translator == 'SpecificJob' && (Job::checkParticularJob($userId, $oneJob) != 'userCanNotAcceptJob') && $this->isNeedToDelayPush($oneUser->id)) {
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        if (($job->customer_physical_type == 'no' && $job->customer_phone_type == 'yes') || ($job->customer_physical_type == 'yes' && $job->customer_phone_type == 'yes')) {
            // It's a phone job
        ...
        foreach ($translators as $translator) {
            ...
            logger('Send SMS to ' . $translator->email . ' (' . $translator->mobile . '), status: ' . print_r($status, true));
        }
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        if ($data['notification_type'] == 'suitable_job') {
            // these hard coded values should be saved in config files
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        // this is repeatedly using in the same file, it must be a function which return a transaction type. or accessor/mutator can be used to handle this
        if ($job_type == 'paid')
        ...
        if (!empty($job->certified)) {
            ...
            // this is useless and unreachable code,
    ...
    public function acceptJob($data, $user)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                $subject = 'Bekräftelse - tolk har accepterat er bokning (bokning # ' . $job->id . ')';
                $name = $user->name;
                $mailer->send($email, $name, $subject, 'emails.job-accepted', compact('user', 'job'));
            }
            /*@todo
...
//                event(new JobWasCanceled($job));
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        foreach ($job_ids as $k => $job) {
            ...
            $ifSpecificAndParticularJob = $job->specific_job == 'SpecificJob' && $job->check_particular_job == 'userCanNotAcceptJob';
            $checkCustomerPhoneType = $job->customer_phone_type == 'no' || $job->customer_phone_type == '';
            if ($ifSpecificAndParticularJob  || ($checkCustomerPhoneType && $job->customer_physical_type == 'yes' && $checktown == false)) {
                unset($job_ids[$k]);
            }
        }
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function endJob($post_data)
    {
        ...
        $session_time = $session_explode[0] . ' tim ' . $session_explode[1] . ' min';
        $mailer = new AppMailer();
        $mailer->send($email, $name, $subject, 'emails.session-ended', array_merge(compact('user', 'job', 'session_time'), ['for_text' => 'faktura']));
        $job->save();
        $tr = $job->translatorJobRel()->where('completed_at', Null)->where('cancel_at', Null)->first();
        ...
        $mailer = new AppMailer();
        $mailer->send($email, $name, $subject, 'emails.session-ended', array_merge(compact('user', 'job', 'session_time'), ['for_text' => 'lön']));
        $tr->completed_at = $completeddate;
